...
def get_predictions(model, param_grid, X_train, Y_train_loo, X_test):
    grid_model = GridSearchCV(model, param_grid, cv=5, scoring="roc_auc_ovr", n_jobs=-10, verbose=0)
...
for model_name, (use_model, model, param_grid) in models_info.items():
    ...
    for left_out_id in tqdm(np.unique(baseline_id_label), desc=f"{model_name} LOO Progress"):
        ...
        X_train = [s for i, s in enumerate(baseline_samples) if baseline_id_label[i] != left_out_id]
        ...
        augmented_samples, augmented_labels = generate_augmented_dataset(
                X_train, Y_train_loo, augmenter, N, augmenter.proportional_shifting, p0=proportional_shifting_p0
            )
        augmented_samples, augmented_labels = generate_augmented_dataset(
            augmented_samples, augmented_labels, augmenter, N, augmenter.machine_variability, v3=machine_variability_v3
        )
        
        X_train = [augmented_samples.intensity for augmented_samples in augmented_samples]
...
all_results_df.to_csv("results/test_case_results_table3.csv", index=False)
