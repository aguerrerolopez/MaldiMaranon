# ------------------------------------------------------------------------------
#        MALDI-TOF SPECTRAL VARIABILITY EXPERIMENTS WITH BASELINE ALIGNMENT
#
# SUMMARY:
# This script analyzes the robustness of MALDI-TOF mass spectrometry biomarkers
# for Clostridioides difficile (C. diff) across different culture media, time points,
# and extraction protocols, using both spectrum alignment and feature selection.
#
# KEY FEATURES:
# - Loads spectra (mzML/Bruker) from structured experimental folders.
# - Aligns every sample spectrum to the mean baseline spectrum using pyOpenMS,
#   correcting for m/z axis drifts and technical variability.
# - Applies a preprocessing pipeline: variance stabilization, smoothing,
#   baseline correction, normalization, trimming, binning, and intensity thresholding.
# - Trains a one-vs-rest Random Forest classifier per ribotype on baseline samples
#   (flexibly chosen, e.g., "Medio Sc", "NoPE", or "PE").
# - Extracts the most discriminative features ("biomarkers") for each ribotype.
# - Evaluates biomarker variability via t-tests across all combinations of
#   media, timepoints, and extraction protocols, comparing both peak intensity
#   and position (centroid).
# - Plots group means and standard deviations, always overlaying the baseline
#   mean spectrum (in blue, dotted line) for reference in each plot.
# - Outputs results as CSV files and publication-ready figures.
#
# PURPOSE:
# The script is designed to test the biological and technical robustness of
# ML-discovered MALDI-TOF biomarkers under realistic laboratory variability.
# The alignment step enables cross-condition comparability, while flexible
# evaluation and visualization support reproducibility and transparent reporting.
#
# USAGE:
# - Adapt baseline parameters or evaluation modes at the top of the script.
# - Requires: numpy, pandas, scikit-learn, pyOpenMS, pymzml, matplotlib.
# - Run from project root or ensure PYTHONPATH includes root directory.
# ------------------------------------------------------------------------------
...
if __name__ == '__main__':
    ...
    results_df = variability_t_tests(all_samples, biomarkers_info, window=window, baseline_media=baseline_media, baseline_week=baseline_week, baseline_extraction=baseline_extraction, csv_name="t_test_variability_results_automaticpeaks_by_RF.csv")
